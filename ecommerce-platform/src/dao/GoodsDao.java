...
public class GoodsDao {
    ...
    public List<Map<String,Object>> getGoodsList(int recommendType) throws SQLException {
        ...
        String sql="select g.id,g.name,g.cover,g.price,t.name typename from recommend r,goods g," +
                "type t where type=? and r.goods_id=g.id and g.type_id=t.id";
    ...
    public List<Goods> selectGoodsbyRecommend(int type,int pageNumber,int pageSize)
        ...
        if(type==0) {
            // 当不添加推荐类型限制的时候
            String sql = " select g.id,g.name,g.cover,g.image1,g.image2,g.intro,g.price," +
                    "g.stock,t.name typename from goods g,type t where g.type_id=t.id " +
                    "order by g.id limit ?,?";
            return r.query(sql, new BeanListHandler<Goods>(Goods.class),
                    (pageNumber-1)*pageSize,pageSize);
        }
        String sql = " select g.id,g.name,g.cover,g.image1,g.image2,g.intro,g.price," +
                "g.stock,t.name typename from goods g,recommend r,type t " +
    ...
    public Goods getGoodsById(int id) throws SQLException {
        ...
        String sql = "select g.id,g.name,g.cover,g.image1,g.image2,g.price,g.intro,g.stock," +
                "t.id typeid,t.name typename from goods g,type t where g.id = ? and g.type_id=t.id";
    ...
    public void insert(Goods g) throws SQLException {
        ...
        String sql = "insert into goods(name,cover,image1,image2,price,intro,stock,type_id) " +
                "values(?,?,?,?,?,?,?,?)";
        r.update(sql,g.getName(),g.getCover(),g.getImage1(),g.getImage2(),g.getPrice(),g.getIntro(),
                g.getStock(),g.getType().getId());
    }

    public void update(Goods g) throws SQLException {
        QueryRunner r = new QueryRunner(JDBCUtils.getDataSource());
        String sql = "update goods set name=?,cover=?,image1=?,image2=?,price=?,intro=?,stock=?," +
                "type_id=? where id=?";
        r.update(sql,g.getName(),g.getCover(),g.getImage1(),g.getImage2(),g.getPrice(),g.getIntro(),
